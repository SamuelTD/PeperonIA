...
onlyfiles = [f for f in os.listdir("assets/") if os.path.isfile(os.path.join("assets/", f))]
batch_size=50
index = 0
# Charge et découpe le PDF
for file in onlyfiles:
    pdf_chunks = load_and_chunk_pdf(f"assets/{file}")
    # collection.add(documents=pdf_chunks, ids=[f"{index}_chunk_{i}" for i in range(len(pdf_chunks))])
    for i in range(0, len(pdf_chunks), batch_size):
        batch = pdf_chunks[i : i + batch_size]
        ids   = [f"{index}_chunk_{j}" for j in range(i, i + len(batch))]
